...
def scrape_jp_weekly_recap(vector_store: Provider[Container.vector_store_recap]):
    ...
    collection = Collection(name=os.getenv("MILVUS_COLLECTION_NAME_RECAP", "weekly_recap"))
    collection.load()
    try:
    ...
    # 메타데이터 생성
    metadatas = [{"hash": hash, "created_at": datetime.datetime.now().isoformat()} for _ in range(len(chunks))]
...
def create_collection_if_not_exists():
    ...
    if not has_collection:
        ...
        fields = [
            ...
            FieldSchema(name="created_at", dtype=DataType.VARCHAR, max_length=30),  # ISO 형식 UTC 시간
            FieldSchema(name="vector", dtype=DataType.FLOAT_VECTOR, dim=1536),  # OpenAI text-embedding-3-small 모델의 차원
